...
* 转为二进制：1111010010&10001001101，比较结果为：1000000转为十进制： 64。所以1010&1101=64；
*  ~ ：0变1,1变0
*  ^ ：两边的位不同时，结果为1，否则为0.如1100^1010=0110
* | ：当两边操作数的位有一边为1时，结果为1，否则为0。如1100|1010=1110
* << 带符号左移 >>带符号右移,左边空出的位以0填充 >>> 无符号右移
...
public class DES {
    ...
    //置换IP
    //为把64位明文按照表中的规则替换，比如第一行，把64位明文的第1位比特换为其比特串中的58位，第2位换为50位....
    ...
    /**
     *S盒。Feistl函数中的一个关键步骤是根据此表执行位替换。
     *48位值被分成6位段，每个部分根据这8个表被转换成不同的6位值（每个节一个表）
     *
     * According to Wikipedia:
     * “S盒是DES安全性的核心——没有它们，密码将是线性的，而且很容易被破解。”
     */
    ...
    /**
     * “P”排列。Feistel函数的结论是将这个32位置换应用于S盒替换的结果，
     *以便在下一轮中将输出位分散到6个不同的S盒中。
    ...
    /**
     * 根据所提供的置换表，置换srcWidth位的输入值“src”。（请注意，我们的排列表由Wikipedia提供，开始以最左边的位为“1”。）
    ...
    private static byte[] parseBytes(String s) {
        s = s.replace(" ", "");//将字符串里的空格字符替换为空
        byte[] ba = new byte[s.length()/2];
        if (s.length()%2 > 0) { s = s+'0'; }
        for (int i=0; i<s.length(); i+=2) {
            //chatAt()函数，返回 char指定索引处的值。
